...
/**
 * Подія одного обчислення флага — контракт для 03d/analytics (спека §6.4).
 * Серіалізується Jackson-ом у JSON value Kafka-повідомлення; key повідомлення —
 * {@link #flagKey}, header — {@code schema-version: 1}.
 *
 * <p>{@code @RegisterForReflection} обов'язковий (камінь #1): Jackson у
 * native-image читає record рефлексією, і без реєстрації GraalVM мовчки
 * викидає member-и — подія серіалізується в {@code {}}.
 *
 * @param evaluationId   UUIDv7 ({@link Uuid7}) — ідемпотентність у analytics (A-04)
 ...
 * @param userKeyHash    SHA-256 hex від userKey — PII не покидає edge (E5)
