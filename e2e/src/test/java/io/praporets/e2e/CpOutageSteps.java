 ...
 * сам наздоганяє після воскресіння.
 *
 * <p><b>Стан між кроками:</b> поля тримають ревізію на момент падіння і
 * baseline метрики застарілості. «Edge-репліка» сценарію — репліка 0;
 * решта реплік живуть поруч і теж переживають падіння — сценарій свідомо
 * дивиться на одну.
 *
 ...
 * Останній Тоді-крок це гарантує.
 */
public class CpOutageSteps {
    ...
    /**
     * Піднімає стек і ставить синхронізаційний бар'єр: очікування (60с),
     * доки ревізія репліки 0 не дорівняється
     * {@link E2eStack#currentCpRevision()}. Бар'єр обов'язковий на
     * спільному стеку: попередній сценарій може зробити PUT config і НЕ
     * чекати доїзду дельти до edge — без бар'єра ревізія, знята тут,
     * гонила б із дельтою в польоті і Тоді-асерт «та сама ревізія» флакав
     * би. ПІСЛЯ бар'єра фіксуються ревізія і baseline метрики
     * застарілості ({@link E2eStack#STALENESS_METRIC}).
     */
    ...
    /**
     * Зупиняє control-plane. Postgres/Kafka/edge-и живі — падає рівно
     * control-plane (модель: деплой/аварія CP у проді).
     */
    ...
    /**
     * Вибір AP наочно: обчислення на репліці 0 повертає 200 із ТИМ
    ...
    /**
     * Чекає (дедлайн 20с — понад heartbeat-інтервал 15с: heartbeat
     * між зняттям baseline і падінням міг скинути staleness у нуль, і рости
     * до baseline метриці доведеться заново), доки метрика застарілості
     * репліки 0 не стане строго більшою за baseline з Дано-кроку —
     ...
     * до 15с) — тому асерт «більше за baseline», а не «більше за нуль».
    ...
    /**
     * Піднімає control-plane заново — той самий host-порт REST і той
     ...
     * ревізія без змін не росте): {@code toggleFlag(false)} → очікування
     * (дедлайн 60с: кап reconnect-backoff-у 30с + DNS-кеш JVM до 30с), доки
     * ревізія репліки 0 не дорівняється {@link E2eStack#currentCpRevision()}
     * і reason не стане {@code FLAG_DISABLED}. Потім {@code toggleFlag(true)}
     * + таке саме очікування ПО ВСІХ репліках — інваріант здорового стека
     * для наступних сценаріїв (див. JavaDoc класу).
     */
